...
class Todo(Resource):
    ...
    def put(self, todo_id):
        print("debug: put as a post? '{}'".format(todo_id))
        args = parser.parse_args()
        task = {'task': args['task']}
        TODOS[todo_id] = task
        return task, 201
...
class ObtainStudentData(Resource):
    ...
    def get(self, student_id):
        print("debug: getting task with the id '{}'".format(student_id))
        #abort_if_todo_doesnt_exist(todo_id)
        return TODOS[student_id], 200
...
api.add_resource(ObtainStudentData, '/studentid/<student_id>')
